...
import { Home, BookOpen, User, LogIn, LogOut, Menu, X, ChevronLeft, ChevronRight, ArrowLeft, ArrowRight } from "lucide-react";
...
const Header = ({ hideHeader = false }) => {
  ...
  useEffect(() => {
    const noNavPages = ["/", "/course", "/admin/login"];
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (storedUser && (!role || role !== "admin" || adminToken))  {
        try {
        ...
        // If adminToken is missing but role is still admin
        setRole(null);
      }
    }, 500);
  ...
  const handleHomeClick = (e) => {
    ...
    navigate(storedRole === "user" ? "/course" : "/");
  };
    ...
    return (
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-zinc-800 border-t border-zinc-700 z-50">
        <div className="flex justify-around py-2">
           <button 
            onClick={handleHomeClick} // Use the same handleHomeClick function
            className={`flex flex-col items-center p-2 ${isActive('/') || isActive('/course') ? 'text-amber-400' : 'text-white'}`}
          >
            <Home size={20} />
          ...
          <Link 
            to="/study-materials" 
            className={`flex flex-col items-center p-2 ${isActive('/study-materials') ? 'text-amber-400' : 'text-white'}`}
          ...
          <Link 
            to="/syllabus" 
            className={`flex flex-col items-center p-2 ${isActive('/syllabus') ? 'text-amber-400' : 'text-white'}`}
            onClick={() => setIsOpen(false)}
          >
            <BookOpen size={20} />
          ...
          <div className="flex items-center space-x-4">
            {showNavButtons && (
              <div className="flex space-x-2">
                <button
                  onClick={goBack}
                  className="w-10 h-10 flex items-center justify-center bg-zinc-700 rounded-full shadow-md hover:bg-amber-500 transition-colors"
                  aria-label="Go back"
                >
                  <ChevronLeft size={20} />
                </button>
            ...
            <div 
              ...
              className="text-2xl font-bold cursor-pointer flex items-center px-4 py-1 rounded-lg hover:bg-zinc-600 transition-colors"
            >
              <span className="text-amber-400">JOYFUL</span> 
              <span className="text-white"> GENIUS</span>
            </div>
          </div>

          {role === "user" && userName && (
            <span className="text-sm text-amber-300 mr-4">
      ...
      <header className="md:hidden bg-zinc-800 text-white shadow-md sticky top-0 z-40">
        ...
        <div className={`bg-zinc-700 transition-all duration-300 ${isOpen ? "max-h-screen py-4" : "max-h-0 overflow-hidden"}`}>
          <div className="container mx-auto px-4 flex flex-col space-y-3">
            ...
            {role === "admin" ? (
              <button onClick={() => { handleLogout(); setIsOpen(false); }} className="text-left py-2 px-4 hover:bg-zinc-600 rounded-lg text-red-400">
                Logout
              </button>
            ) : role === "user" ? (
              <Link to="/dashboard" onClick={() => setIsOpen(false)} className="py-2 px-4 hover:bg-zinc-600 rounded-lg">
                Dashboard
              </Link>
            ) : (
              <Link to="/admin/login" onClick={() => setIsOpen(false)} className="py-2 px-4 hover:bg-zinc-600 rounded-lg">
                Admin Login
              </Link>
            )}
          </div>
