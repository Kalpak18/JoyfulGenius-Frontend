...
//   Legend,
// } from "recharts";
// import { Home, BookOpen, User, ChevronLeft, ChevronRight, ChevronDown,Trash2,
...
//     try {
//       const res = await api.patch("/users/update-email", { email: newEmail }, {
//         headers: { Authorization: `Bearer ${token}` },
//       });
...
//       fetchUser();
//     } catch (err) {
//       setEmailStatus("❌ Failed to update email");
//     }
//   };
...
//                 <MailPlus size={16} className="inline-block mr-1" /> Add Email
//               </button>
                  ...
                  <div className="bg-white p-4 rounded shadow">
                    ...
                    {filteredAutoTests.length === 0 ? (
                      ...
                      <>
                        <ul className="space-y-3">
                          {filteredAutoTests.map((t, i) => (
                            <li key={t._id || i} className="bg-gray-50 p-3 rounded shadow">
                              <p><strong>Subject:</strong> {t.subject}</p>
                              <p><strong>Chapter:</strong> {t.chapter}</p>
                              <p><strong>Score:</strong> {t.score} / {t.total}</p>
                              <p><strong>Date:</strong> {new Date(t.createdAt).toLocaleString()}</p>
